...
const Navbar = () => {
  ...
  const handleAdminRequest = async () => {
    try {
      ...
      const res = await axios.post(
        "http://localhost:5000/api/auth/admin-request",
        {},
  ...
  return (
    ...
    {/* Right Drawer */}
    <div
      className={`fixed top-0 right-0 h-full w-72 bg-white z-50 p-6 shadow-lg transform transition-transform duration-300 ${
    ...
    >
      ...
      <div className="flex items-center justify-between mt-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <p className="text-sm  text-gray-800">Account</p>
      ...
      <ul className="flex flex-col mt-6 space-y-6 text-black text-lg font-medium">
        ...
        {user && (
          <button
            ...
            className="text-left font-bold text-xl text-red-600"
          >
            Logout
          </button>
        )}
      ...
      {!user && (
        <button
          ...
          className="text-green-700 font-bold text-xl hover:text-green-800 cursor-pointer mt-4"
        >
          Login
        </button>
      )}
